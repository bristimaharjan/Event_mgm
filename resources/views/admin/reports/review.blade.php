...
<div class="max-w-7xl mx-auto mt-10 ml-72 mr-10">
    ...
    <thead>
        <tr class="bg-[#8d85ec] text-white">
            ...
            <th class="px-6 py-3 w-64">Venue Details</th>
            <th class="px-6 py-3 w-36">User</th>
            <th class="px-6 py-3 w-24">Rating</th>
            <th class="px-6 py-3 w-64">Comment</th>
            <th class="px-6 py-3 w-28">Date</th>
            <th class="px-6 py-3 w-20 text-center rounded-tr-lg">Action</th>
        </tr>
    </thead>
    <tbody>
        @forelse($reviews as $review)
            <tr class="border-b hover:bg-gray-50 transition">
                ...
                <!-- User -->
                <td class="px-6 py-4 break-words">{{ $review->user->name ?? '-' }}</td>
                ...
                <!-- Action -->
                <td class="px-6 py-4 whitespace-nowrap">
                    <form action="{{ route('venueReview.destroy', $review->id) }}" method="POST" onsubmit="return confirm('Are you sure?');">
                        @csrf
                        @method('DELETE')
        ...
        @empty
            <tr>
                <td colspan="7" class="px-6 py-6 text-center text-gray-500">No reviews found.</td>
            </tr>
        @endforelse
    </tbody>
</table>
